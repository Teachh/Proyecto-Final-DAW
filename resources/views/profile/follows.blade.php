...
@foreach($followers as $follower)
    ...
    <a href="{{action('UserController@show', App\User::findOrFail($follower->user_id)->id)}}">
        <div class="details">
            <h2>{{App\User::findOrFail($follower->user_id)->name}}</h2>
            <div class="product">
                <img class="cardimg" src="{{asset(App\User::findOrFail($follower->user_id)->profile_picture)}}">
            </div>
        </div>
    </a>
    </li>
